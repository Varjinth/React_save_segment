...
{schemas.filter((schema) => !array.some((ele) => Object.values(ele)[0] === Object.values(schema)[0])).map((schema, index) => (
       
        <div key = {index} 
        // className={`${schemaOptionClass(schema)}`}
        className='dropdown-option-wrapper' 
        onClick={()=>{
            setOpenDropdown(false);
    props.setSelected(schema);
